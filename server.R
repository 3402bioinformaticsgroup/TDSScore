...
output$plotDrugScore<-renderPlot({
    ...
    rankMatrix<-as.matrix(data.gene[,4]*data.gene[,1])
    rownames(rankMatrix)<-rownames(data.gene)
    colnames(rankMatrix)<-"tds.fc"
    rankMatrix<-as.matrix(rankMatrix[order(rankMatrix[,1],decreasing=T),])
    rankMatrix[,1]<-rank(-rankMatrix[,1])   # convert tds logFC to a rank matrix, increasing order;
                                           # small number tends to be up-regulated genes
    scores<-matrix(rep(0,dim(drug.table)[2]*4), ncol=4)
    colnames(scores)<-c("Ks.up","Ks.dn","S","score")
    rownames(scores)<-colnames(drug.table)
    
    for (i in 1:dim(drug.table)[2]){
        n<-dim(rankMatrix)[1]
        current.drug<-as.matrix(drug.table[,i])   #
        tds.up<-as.matrix(subset(current.drug, current.drug[,1] >0))
        tds.up<-cbind(rownames(tds.up), tds.up)
        tds.dn<-as.matrix(subset(current.drug, current.drug[,1] <0))
        ...
        instance.i<-rankMatrix  # sorted tds fc put in order
    ...
    ######
    plot(drug.score[,6], main="Drug Score",pch=1,xlab="Drug Index",ylab="TDS score",
    ylim=c(min(drug.score[,6])-1, max(drug.score[,6])+1))
    points(drug.score[,5],pch=16)
    legend("topright",
